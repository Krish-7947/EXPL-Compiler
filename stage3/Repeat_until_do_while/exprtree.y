...
%token <node> START END READ WRITE PLUS MINUS MUL DIV ASSGN NUM ID REPEAT UNTIL
...
%type <node> program Slist Stmt InputStmt OutputStmt AsgStmt expr IfStmt WhileStmt BrkStmt ContStmt RepeatUntilStmt DoWhileStmt
...
RepeatUntilStmt : REPEAT Slist UNTIL '(' expr ')' ';'   {
                                                            typecheck($5->type, TYPE_BOOL, 'r');
                                                            $$ = createTree(TYPE_VOID, 0, NODE_REPEAT_UNTIL, NULL, $5, $2, NULL);
                                                        }

DoWhileStmt : DO Slist WHILE '(' expr ')' ';'           {
                                                            typecheck($5->type, TYPE_BOOL, 'd');
                                                            $$ = createTree(TYPE_VOID, 0, NODE_DO_WHILE, NULL, $5, $2, NULL);
                                                        }
