...
int getMemoryAddress(struct tnode* t) {
    ...
    if(t->nodetype == NODE_ID) {
        ...
        return r;
    } else if(t->nodetype == NODE_ARRAY2D) {
        rowReg = codegen(t->middle);           // row index value
        colReg = codegen(t->right);            // col index value
        dimReg = getReg();
        fprintf(intermediate, "MOV R%d, %d\n", dimReg, t->left->Gentry->cols);
        fprintf(intermediate, "MUL R%d, R%d\n", rowReg, dimReg);   // rowReg = row * cols
        freeReg();                              // free dimReg (top of the register "stack")
        fprintf(intermediate, "ADD R%d, R%d\n", rowReg, colReg);   // rowReg = row*cols + col
        freeReg();                              // free colReg
        fprintf(intermediate, "ADD R%d, %d\n", rowReg, t->left->Gentry->binding); // + base
        return rowReg;
    } else {
        return -1;
    }
